    ...
    for page in rootpage:
        ...
        for item in playerlist("tbody > tr"):
            try:
                ...
                i=0
                if 'Name im Heimatland' in playerpage("table.tabelle_spieler.s10 tr").eq(0)("td:first").text():
                    original_name = playerpage("table.tabelle_spieler.s10 tr").eq(0)("td:last").text()
                    i=1
                date_of_birth = playerpage("table.tabelle_spieler.s10 tr").eq(0+i)("td:last").text()
                age = playerpage("table.tabelle_spieler.s10 tr").eq(2+i)("td:last").text()
                size = playerpage("table.tabelle_spieler.s10 tr").eq(3+i)("td:last").text()
                size = size.replace(',','.')
                position = PyQuery(playerpage("table.tabelle_spieler.s10 tr")[5+i])("td:last").text()
                try:
                    position = POSITION_MAPPING[position]
                except:
                    print name, nation
                foot = playerpage("table.tabelle_spieler.s10 tr").eq(6+i)("td:last").text()
                value = playerpage("table.tabelle_spieler.s10 tr").eq(7+i)("td:last").text()
                ...
                if not session.query(Player).filter(Player.name==name and Player.event.id_==event).all():
                    player = Player(name, position, nation_id, event, original_name=original_name, date_of_birth=date_of_birth, age=age, foot=foot, value=value, size=size, club=clubname, image=img)
                    session.add(player)
            except:
                print 'fuck you'
    transaction.commit()
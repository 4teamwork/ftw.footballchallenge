...
from ftw.footballchallenge.player import get_keeper_term, get_defender_term, get_midfield_term, get_striker_term
...
#And now the Substitutes
    ...
    def player_only_once(data):
        keys = EditTeamSchema
        if len(set([getattr(data, name) for name in keys.names()])) == len(keys.names()):
            return True
    ...
    def handleEdit(self, action):
        ...
        if len(errors) == 0:
            ...
            if not session.query(Team).filter(Team.user_id==userid).all():
                ...
                team = session.query(Team).filter(Team.name==data['name'] and Team.userid==userid and Team.event_id == event_id)
            session.query(Teams_Players).filter(Teams_Players.team_id==team.id_).delete()
